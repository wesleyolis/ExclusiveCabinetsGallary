...
<% End If %>  
	...
	<a href="Descriptionslist.asp?order=<%= Server.URLEncode("Description") %>" style="color: #FFFFFF;">Description<% If Session("Descriptions_x_Description_Sort") = "ASC" Then %><img src="images/sortup.gif" width="10" height="9" border="0"><% ElseIf Session("Descriptions_Description_Sort") = "DESC" Then %><img src="images/sortdown.gif" width="10" height="9" border="0"><% End If %></a>
			</span></td>
...
<% Response.Write x_Description %>
		...
		<tr>
			<td bgcolor="#3366CC"><span class="aspmaker" style="color: #FFFFFF;">
	<a href="Descriptionslist.asp?order=<%= Server.URLEncode("Name") %>" style="color: #FFFFFF;">Name<% If Session("Descriptions_x_Name_Sort") = "ASC" Then %><img src="images/sortup.gif" width="10" height="9" border="0"><% ElseIf Session("Descriptions_Name_Sort") = "DESC" Then %><img src="images/sortdown.gif" width="10" height="9" border="0"><% End If %></a>
			</span></td>
			<td bgcolor="#FFFFFF"><span class="aspmaker">
...
<span class="aspmaker">
	<a href="<% If Not IsNull(sKey) Then Response.Write "Descriptionsview.asp?key=" & Server.URLEncode(sKey) Else Response.Write "javascript:alert('Invalid Record! Key is null');" End If %>">View</a>&nbsp;
	<a href="<% If Not IsNull(sKey) Then Response.Write "Descriptionsedit.asp?key=" & Server.URLEncode(sKey) Else Response.Write "javascript:alert('Invalid Record! Key is null');" End If %>">Edit</a>&nbsp;
	<a href="<% If Not IsNull(sKey) Then Response.Write "Descriptionsadd.asp?key=" & Server.URLEncode(sKey) Else Response.Write "javascript:alert('Invalid Record! Key is null');" End If %>">Copy</a>&nbsp;
...
<p><input type="button" name="btndelete" value="DELETE SELECTED" onClick="this.form.action='Descriptionsdelete.asp';this.form.encoding='application/x-www-form-urlencoded';this.form.submit();"></p>
<% End If %>
</form>
<%
...
Sub SetUpDisplayRecs()
	...
	If sWrk <> "" Then
		...
		Else
			...
			End If
		End If
		Session("Descriptions_RecPerPage") = nDisplayRecs ' Save to Session

		' Reset Start Position (Reset Command)
		nStartRec = 1
		Session("Descriptions_REC") = nStartRec
	Else
		If Session("Descriptions_RecPerPage") <> "" Then
			nDisplayRecs = Session("Descriptions_RecPerPage") ' Restore from Session
		Else
...
Sub SetUpSortOrder()
	...
	If Request.QueryString("order").Count > 0 Then
		...
		If sOrder = "Description" Then
			...
			sLastSort = Session("Descriptions_x_Description_Sort")
			If sLastSort = "ASC" Then sThisSort = "DESC" Else sThisSort = "ASC"
			Session("Descriptions_x_Description_Sort") = sThisSort
		Else
			If Session("Descriptions_x_Description_Sort") <> "" Then Session("Descriptions_x_Description_Sort") = ""
		End If

		' Field Name
		If sOrder = "Name" Then
			sSortField = "[Name]"
			sLastSort = Session("Descriptions_x_Name_Sort")
			If sLastSort = "ASC" Then sThisSort = "DESC" Else sThisSort = "ASC"
			Session("Descriptions_x_Name_Sort") = sThisSort
		Else
			If Session("Descriptions_x_Name_Sort") <> "" Then Session("Descriptions_x_Name_Sort") = ""
		End If
		Session("Descriptions_OrderBy") = sSortField & " " & sThisSort
		Session("Descriptions_REC") = 1
	End If
	sOrderBy = Session("Descriptions_OrderBy")
	If sOrderBy = "" Then
		sOrderBy = sDefaultOrderBy
		Session("Descriptions_OrderBy") = sOrderBy
	End If
End Sub
...
Sub SetUpStartRec()
	...
	ElseIf Request.QueryString("pageno").Count > 0 Then
		...
		If IsNumeric(nPageNo) Then
			...
			Session("Descriptions_REC") = nStartRec
		Else
			nStartRec = Session("Descriptions_REC")
...
Sub ResetCmd()
	...
	If Request.QueryString("cmd").Count > 0 Then
		...
		If UCase(sCmd) = "RESET" Then
			sSrchWhere = ""
			Session("Descriptions_searchwhere") = sSrchWhere
