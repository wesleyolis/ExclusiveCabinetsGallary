...
<form method="post" encType="multipart/form-data" onSubmit="return cheack_filetype();" action="ToFileSystem.asp?d=<%=Request.QueryString("d")%>&Key=<%=Request.QueryString("Key")%>">
<table border="1" cellpadding="3" style="border-collapse: collapse" class="text">
	...
	<tr>
		...
		<input  class="text" onclick="document.location.replace('DirBrowser.asp?d=<%=Request.QueryString("d")%>')" type="button" value="Cancel" name="Cancel">&nbsp;&nbsp;
		<%End If%>
