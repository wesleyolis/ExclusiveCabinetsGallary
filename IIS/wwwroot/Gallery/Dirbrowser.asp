...
FIL = FIL & " AND (InStr(UCase(([Images.Image])), UCase('" & Client & "')) > 0)"  '' " AND (UCase(([Images.Image]))=UCase('" & Client & "')) "
End IF
...
'Response.write(FIL)
sql = "SELECT Images.Image, Images.Code, Images.Client, Ranges.Name, Descriptions.Name, Colors.Name, Images.Color, Images.Width, Images.Height, Images.Depth,IIF([Images.Price]=0,'P.O.A','R ' & [Images.Price]) As Price,Images.Edge, Images.Info"_
...
<b>G.C </b>&nbsp; <input type="text" name="Client" id="Client" size="16" maxlength="16"  value="<%=Request.QueryString("Client")%>">&nbsp;&nbsp; <input type="button" onclick="filter();" value="Search">
</td>
...
<tr>
			...
			<tr>
				...
				<a title="Click here to view Color Sample" href="Colors.asp#jump<%=rsi("Color")%>"><%=rsi.Fields("Colors.Name")%></a>&nbsp;&nbsp; (<a href="Available_Colors.asp?img=<%=rsi.Fields("Image")%>">Available 
		Colours</a>)</td>
			</tr>
			<tr>
			...
			<tr>
				<td width="162" colspan="1"><b>Client Ref:</b>&nbsp;&nbsp;&nbsp; <%=rsi.Fields("Client")%>&nbsp; </td>
				<td ><b>G.C:</b>&nbsp;&nbsp;&nbsp; <%=rsi.Fields("Image")%> </td>
