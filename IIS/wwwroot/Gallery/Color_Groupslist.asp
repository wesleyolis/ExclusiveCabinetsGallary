...
%>
<p><span class="aspmaker">TABLE: Color Groups
<% If sExport = "" Then %>
&nbsp;&nbsp;<a href="Color_Groupslist.asp?export=html">Printer Friendly</a>
&nbsp;&nbsp;<a href="Color_Groupslist.asp?export=excel">Export to Excel</a>
&nbsp;&nbsp;<a href="Color_Groupslist.asp?export=csv">Export to CSV</a>
<% End If %>
</span></p>
<% If sExport = "" Then %>
<table class="ewListAdd">
	<tr>
		<td><span class="aspmaker"><a href="Color_Groupsadd.asp">Add</a></span></td>
	</tr>
</table>
<p>
<% End If %>
...
%>
<% If sExport = "" Then %>
<form action="Color_Groupslist.asp" name="ewpagerform" id="ewpagerform">
...
If nTotalRecs > 0 Then
	...
	If CLng(nTotalRecs) > CLng(nDisplayRecs) Then
		...
		<a href="Color_Groupslist.asp?start=<%=PrevStart%>"><b>Prev</b></a>
		<%
		End If
		...
		<b><%=y%></b>
					<%	Else %>
		<a href="Color_Groupslist.asp?start=<%=x%>"><b><%=y%></b></a>
					<%	End If
					x = x + nDisplayRecs
					y = y + 1
				ElseIf x >= (dx1-nDisplayRecs*nRecRange) And x <= (dx2+nDisplayRecs*nRecRange) Then
					If x+nRecRange*nDisplayRecs < nTotalRecs Then %>
		<a href="Color_Groupslist.asp?start=<%=x%>"><b><%=y%>-<%=y+nRecRange-1%></b></a>
					<% Else
						ny=(nTotalRecs-1)\nDisplayRecs+1
							If ny = y Then %>
		<a href="Color_Groupslist.asp?start=<%=x%>"><b><%=y%></b></a>
							<% Else %>
		<a href="Color_Groupslist.asp?start=<%=x%>"><b><%=y%>-<%=ny%></b></a>
							<%	End If
					End If
		...
		If NOT rsEof Then
			...
			isMore = True %>
		<a href="Color_Groupslist.asp?start=<%=NextStart%>"><b>Next</b></a>
		<% Else
			isMore = False
		End If %>
		<br>
...
Name
<% Else %>
	<a href="Color_Groupslist.asp?order=<%= Server.URLEncode("Name") %>">Name<% If Session(ewSessionTblSort & "_x_Name") = "ASC" Then %><img src="images/sortup.gif" width="10" height="9" border="0"><% ElseIf Session(ewSessionTblSort & "_x_Name") = "DESC" Then %><img src="images/sortdown.gif" width="10" height="9" border="0"><% End If %></a>
<% End If %>
		</span></td>
<% If sExport = "" Then %>
<td>&nbsp;</td>
...
<% If sExport = "" Then %>
<td><span class="aspmaker"><a href="<% If Not IsNull(x_Index) Then Response.Write "Color_Groupsview.asp?Index=" & Server.URLEncode(x_Index) Else Response.Write "javascript:alert('Invalid Record! Key is null');" End If %>">View</a></span></td>
<td><span class="aspmaker"><a href="<% If Not IsNull(x_Index) Then Response.Write "Color_Groupsedit.asp?Index=" & Server.URLEncode(x_Index) Else Response.Write "javascript:alert('Invalid Record! Key is null');" End If %>">Edit</a></span></td>
<td><span class="aspmaker"><a href="<% If Not IsNull(x_Index) Then Response.Write "Color_Groupsdelete.asp?Index=" & Server.URLEncode(x_Index) Else Response.Write "javascript:alert('Invalid Record! Key is null');" End If %>">Delete</a></span></td>
<td><span class="aspmaker"><a href="Colors_Grouplist.asp?showmaster=1&Grp=<%= Server.URLEncode(x_Index)%>">Colors Group Details</a></span></td>
<% End If %>
	</tr>
<%
	End If
