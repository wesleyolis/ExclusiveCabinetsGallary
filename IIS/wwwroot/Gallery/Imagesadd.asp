...
Select Case sAction
	...
	Case "E":
		If 	EditData(x_Image) Then ' Add New Record
			...
			Color_Ranges(x_Image)
			conn.Close ' Close Connection
			Set conn = Nothing
			Response.Clear
			Response.Redirect "Sub_Browser.asp?I=" & x_Image
		End If
...
<form name="FrontPage_Form1" id="Imagesadd" action="Imagesadd.asp" method="POST" onSubmit="return FrontPage_Form1_Validator(this)" language="JavaScript">
<p>
...
<!--webbot bot="Validation" s-data-type="String" b-allow-letters="TRUE" b-allow-digits="TRUE" i-maximum-length="16" --><input  class="input" type="text" name="x_Code" id="x_Code" size="30" maxlength="16" value="<%= Server.HTMLEncode(x_Code&"") %>" tabindex="1">
		...
		<td bgcolor="#F5F5F5" width="10" rowspan="8">
&nbsp;</td>
	<td  bgcolor="#3366CC" width="369"  class="box2" height="40">Available 
		Colours</td>	
	</tr>
	<tr>
		<td bgcolor="#3366CC" height="29" class="box2" width="58">Client Ref:</td>
		<td bgcolor="#F5F5F5" height="29" width="216">
		...
		<td  bgcolor="#F5F5F5" width="369" height="40" rowspan="3">
		<select size="5" name="ColorsAv" multiple style="width: 364; height: 89">
		...
		Set rs = conn.Execute("SELECT r.Index, r.Name, Max(r.selected) AS Selected FROM (SELECT Color_Groups.Index, Color_Groups.Name, '0' AS selected FROM Color_Groups union SELECT Color_Groups.Index, Color_Groups.Name, '1' AS Selected FROM Image_Groups INNER JOIN Color_Groups ON Image_Groups.Group = Color_Groups.Index WHERE (((Image_Groups.Image)=" & simg & ")) ) AS r GROUP BY r.Index, r.Name;")
		...
		%><option <%=sel%> value="<%=rs("Index")%>"><%=rs("Name")%></option><%
		rs.MoveNext()		
		Loop		
...
<input  class="text" type="text" name="x_Depth" id="x_Depth" size="5" value="<%= Server.HTMLEncode(x_Depth&"") %>" style="height: 18" tabindex="8"></td>
	<td bgcolor="#F5F5F5" rowspan="4" width="369"><span class="aspmaker">
<textarea  class="text" cols="43" rows="15" id="x_Info" name="x_Info" style="width: 366; height: 114" tabindex="11"><%= x_Info %></textarea></span></td>
	</tr>
	<tr>
...
<% If IsNull(x_Price) or x_Price = "" Then x_Price = 0 ' Set default value %><input  class="input" type="text" name="x_Price" id="x_Price" size="30" value="<%= Server.HTMLEncode(x_Price&"") %>" tabindex="9">
	...
	<tr>
		<td bgcolor="#F5F5F5" width="252" colspan="3" style="height: 34px">
...
Function Color_Ranges(id)

	Set rs = Server.CreateObject("ADODB.Recordset")
	rs.CursorLocation = 3
	sSql = "Select * FROM Image_Groups WHERE (((Image_Groups.Image)=" & id &"))"
	rs.Open sSql, conn, 1, 2
	Do While Not rs.Eof
		rs.Delete
		rs.MoveNext
	...
	sSql = "Select * FROM Image_Groups WHERE 1 = 0"

	Set rs = Server.CreateObject("ADODB.Recordset")
	rs.CursorLocation = 3
	rs.Open sSql, conn, 1, 2
	count = Request.Form("ColorsAv").Count +1
	i=1
	while(i<count)
	
		rs.AddNew
		rs("Image") = id
		rs("Group") = Request.Form("ColorsAv")(i)
	i=i+1
	wend 
	if Not rs.Eof Then
	rs.Update
